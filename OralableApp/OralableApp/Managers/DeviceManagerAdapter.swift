...
final class DeviceManagerAdapter: ObservableObject, BLEManagerProtocol {
    ...
    /// PPG frame numbers are BLE packet counters, so sample timestamps (plus repeated-channel flushes)
    /// are used to avoid collapsing every sample in a multi-sample packet into one row.
    ...
    nonisolated static func biometricSampleArrays(from readings: [SensorReading]) -> (
        ...
        let sorted = readings.stablySortedByTimestamp()
...
private extension Array where Element == SensorReading {
    func stablySortedByTimestamp() -> [SensorReading] {
        enumerated()
            .sorted { lhs, rhs in
...
private extension SensorReading {
    var ppgSampleKey: Int64 {
        Int64((timestamp.timeIntervalSinceReferenceDate * 10_000.0).rounded())
    }
}
