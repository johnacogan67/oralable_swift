...
extension DeviceManager {
    ...
    func handleDeviceConnected(peripheral: CBPeripheral) {
        ...
        // Keep isConnecting true until discovery reaches .ready / .failed (prevents duplicate Connect taps).
    ...
    func discoverServicesAndCharacteristics(peripheral: CBPeripheral) async {
        ...
        let cancelOralableContinuations = {
        ...
        do {
            ...
            try await withTimeout(seconds: 10, onTimeout: cancelOralableContinuations) {
            ...
            if let oralableDevice = device as? OralableDevice {
                do {
                    let deviceId = try await withTimeout(seconds: 5, onTimeout: cancelOralableContinuations) {
                ...
                let version = try await withTimeout(seconds: 5, onTimeout: cancelOralableContinuations) {
            ...
            Logger.shared.debug("[DeviceManager][BLETrace \(traceId)] Step 4/5 enableNRFAlignedStreamingNotifications() start")
            updateDeviceReadiness(peripheral.identifier, to: .enablingNotifications)

            if let oralableDevice = device as? OralableDevice {
                try await withTimeout(seconds: 30, onTimeout: cancelOralableContinuations) {
    ...
    func handleDeviceDisconnected(peripheral: CBPeripheral, error: Error?) {
        ...
        if primaryDevice?.peripheralIdentifier == peripheral.identifier {
            primaryFirmwareDeviceStatus = nil
        }
    ...
    func connect(to deviceInfo: DeviceInfo) async throws {
        ...
        if peripheral.state == .connected {
            Logger.shared.info("[DeviceManager] Already connected to \(deviceInfo.name) — ensuring discovery flow only")
    ...
    func withTimeout<T>(
        ...
        return try await withThrowingTaskGroup(of: T.self) { group in
            ...
            do {
                ...
                group.cancelAll()
                if case DeviceError.timeout = error {
                    onTimeout?()
                }
                throw error
            }
