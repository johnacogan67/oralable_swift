...
class SharedDataManager: ObservableObject {
    ...
    private func updateDayRecord(_ record: CKRecord, with sensorData: [SensorData], patientID: String) async throws {
        ...
        let mergedReadings = Self.mergedSensorReadingsForCloudSync(
            existing: existingReadings,
            incoming: incomingReadings
        )

        if !existingReadings.isEmpty {
            Logger.shared.info("[SharedDataManager] Merged CloudKit day record for \(date): existing=\(existingReadings.count), incoming=\(incomingReadings.count), merged=\(mergedReadings.count)")
        }
