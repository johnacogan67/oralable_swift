...
private enum SharedDataRecordError: LocalizedError {
    case missingCompressedPayloadSize
    case invalidCompressedPayload

    var errorDescription: String? {
        switch self {
        case .missingCompressedPayloadSize:
            return "Existing CloudKit sensor payload is missing its uncompressed size"
        case .invalidCompressedPayload:
            return "Existing CloudKit sensor payload could not be decoded"
        }
    }
}
...
class SharedDataManager: ObservableObject {
    ...
    private func decodeSensorReadings(from record: CKRecord) throws -> [SerializableSensorData] {
        ...
        guard let uncompressedSize = sensorDataUncompressedSize(from: record) else {
            throw SharedDataRecordError.missingCompressedPayloadSize
        }

        guard let decompressedData = compressedData.decompressed(expectedSize: uncompressedSize) else {
            throw SharedDataRecordError.invalidCompressedPayload
        }

        do {
            return try JSONDecoder().decode(BruxismSessionData.self, from: decompressedData).sensorReadings
        } catch {
            Logger.shared.error("[SharedDataManager] Failed to decode existing sensor payload: \(error)")
            throw SharedDataRecordError.invalidCompressedPayload
        }
    }

    private func sensorDataUncompressedSize(from record: CKRecord) -> Int? {
    ...
    static func mergedSensorReadings(
        ...
        func appendIfNew(_ reading: SerializableSensorData) {
