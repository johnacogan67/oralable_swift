...
final class RegressionTests: XCTestCase {
    ...
    func testGuestAuthenticationPersistsAcrossManagerReload() {
        let keys = [
            "isAuthenticated",
            "userID",
            "userFullName",
        ...
        XCTAssertTrue(reloadedManager.isAuthenticated, "Guest authentication should survive app relaunch")
        XCTAssertEqual(reloadedManager.userID, "guest")
        XCTAssertEqual(reloadedManager.userFullName, "Guest")
    }

    func testAccountDeletionClearsPersistedSetupAndClinicalState() async {
        let keys = [
            "isAuthenticated",
            "userID",
            "userFullName",
        ...
        for key in keys {
            XCTAssertNil(UserDefaults.standard.object(forKey: key), "\(key) should be removed during account deletion")
        }
        XCTAssertFalse(manager.isAuthenticated, "Account deletion should leave the current manager signed out")
        XCTAssertNil(manager.userID)
        XCTAssertNil(manager.userFullName)
    }
